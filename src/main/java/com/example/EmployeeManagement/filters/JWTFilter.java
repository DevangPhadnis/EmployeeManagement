...
import com.example.EmployeeManagement.serviceImpl.UserDetailsServiceImpl;
...
public class JWTFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(authorizationHeaders != null && authorizationHeaders.startsWith("Bearer")) {
            ...
            username = jwtUtilis.extractUserName(token);
        }
        if(username != null) {
            UserDetails userDetails = context.getBean(UserDetailsServiceImpl.class).loadUserByUsername(username);
            if(jwtUtilis.validateToken(token)) {
                String role = jwtUtilis.extractUserRole(token);
